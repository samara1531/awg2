# AmneziaWG 2.0-3.1 OpenWRT 
- **awg 3.1 от версии 25.12.5** и выше.
- а так-же в релизе 24.10.8_**v3** но **без модуля Go**
- опция настроек в awg 3.**1** "Random Trailers" может урезать скорость до десяти раз **!!!**.  либо указывайте параметр H1-H3 без диапазона.
+ 
+ На неподдерживаемые архитектуры awg-go не собирается (архивы меньше 1 МБ).
+ 
+ Два варианта установки/работы, через kmod-amneziawg и amneziawg-go
  - если установлен kmod-amneziawg он становится приоритетным, запуск идет через него. даже при наличии awg-go. (это прописано в amneziawg-tools)
+ 
+ Через amneziawg-go, потребляет гораздо больше озу и для хорошей сетевой скорости требует производительного цпу.
  - на стандартном роутере показывает не высокие скоростные параметры.
  - (потребление озу 10-60 Мб на каждый запущенный интерфейс. чем лучше процессор и объемы трафика, тем больше)
+
+ **kmod-amneziawg, на "кастомные" прошивки не установится. только на официальный ванильный wrt**
  - остальные ipk привязаны только к архитектуре процессора.
  - если использовать через awg-go, можно в **теории** поставить на любую сборку.
  - (используйте MTU= **1280**)
+
+ Требуется удалить старые пакеты амнезии, перед установкой новых. критично.
+ Так-же обязательно почистить кеш браузера, дабы корректно отобразился новый интрфс. awg
+
+ для [immortalwrt](https://github.com/samara15321/awg2/releases)
-----------------------
### Автоустановка AWG (если есть пакеты под вашу версию WRT)

 **Для OpenWrt 23.05.6 – 24.10.x**
```sh
opkg update && opkg install unzip
wget -qO- "https://raw.githubusercontent.com/samara1531/awg2/main/autoinstall-awg.sh" | sh
```
**Для OpenWrt 25.12.x**
```sh
apk add unzip
wget -qO- "https://raw.githubusercontent.com/samara1531/awg2/main/autoinstall-awg.sh" | sh
```
**Вариант с AWG-GO (установите unzip)**
```sh
wget -qO- "https://raw.githubusercontent.com/samara1531/awg2/main/autoinstall-GO.sh" | sh
```

------------------------------
+ Вручную 25.12.x
  - т.к. не разрешено пока по умолчанию ставить через вэб сторонние пакеты.
  - вам придется каким-то образом закинуть файлы в папку /tmp/ на роутере (через прогу WinSCP)
  - и ставить пакеты командой `apk add --allow-untrusted /tmp/название_файла.apk`
  - порядок установки: kmod - tools - luci
  - Либо дать разрешение на установку пакетов и установить их через вэб менеджер:
  - `sed -i 's/action="add"/action="add --allow-untrusted"/' /usr/libexec/package-manager-call`
---------------
[Inspired by](https://github.com/this-username-has-been-taken/amneziawg-openwrt)
